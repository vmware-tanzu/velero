...
Node-agent puts advanced configurations of data movement and PodVolume pods into a ConfigMap that contains JSON configuration. The ConfigMap should be created in the same namespace where Velero is installed, and its name is specified using the `--node-agent-configmap` parameter.
...
Controls the concurrent number of data movement and PodVolume operations per node to optimize resource usage and performance.

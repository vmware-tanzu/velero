...
Data mover pods launched during CSI snapshot data movement will inherit the priority class name from the node-agent daemonset if configured during Velero installation using the `--node-agent-priority-class-name` flag. This can help ensure proper scheduling behavior in resource-constrained environments. For more details on configuring data mover pod resources, see [Data Movement Pod Resource Configuration][16].  
...
Note that data mover pods will inherit the priorityClassName from the node-agent daemonset if configured during Velero installation using the `--priority-class-name` flag.   
...
[16]: data-movement-pod-resource-configuration.md

...
func (r *mockRegistry) List(kind plugin.PluginKind) []plugin.PluginIdentifier {
	args := r.Called(kind)
	return args.Get(0).([]plugin.PluginIdentifier)
}

func (r *mockRegistry) Get(kind plugin.PluginKind, name string) (plugin.PluginIdentifier, error) {
	args := r.Called(kind, name)
	var id plugin.PluginIdentifier
	if args.Get(0) != nil {
		id = args.Get(0).(plugin.PluginIdentifier)
	}
...
func TestGetRestartableProcess(t *testing.T) {
	...
	pluginKind := plugin.PluginKindBackupItemAction
	pluginName := "pod"
	...
	getPluginTest(t,
		...
		func(name string, sharedPluginProcess RestartableProcess) interface{} {
			return &restartableObjectStore{
				key:                 kindAndName{kind: plugin.PluginKindObjectStore, name: name},
...
func getPluginTest(
	...
	pluginKind := kind
	pluginName := name
	pluginID := plugin.PluginIdentifier{
		Command: "/command",
		Kind:    pluginKind,
		Name:    pluginName,
	}
...
func TestGetBackupItemActions(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			pluginKind := plugin.PluginKindBackupItemAction
			var pluginIDs []plugin.PluginIdentifier
...
func TestGetRestoreItemActions(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			pluginKind := plugin.PluginKindRestoreItemAction
			var pluginIDs []plugin.PluginIdentifier
			for i := range tc.names {
				pluginID := plugin.PluginIdentifier{
					Command: "/command",

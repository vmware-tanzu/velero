...
type Registry interface {
	...
	List(kind plugin.PluginKind) []plugin.PluginIdentifier
	// Get returns the PluginIdentifier for kind and name.
	Get(kind plugin.PluginKind, name string) (plugin.PluginIdentifier, error)
}
...
type registry struct {
	...
	pluginsByID    map[kindAndName]plugin.PluginIdentifier
	pluginsByKind  map[plugin.PluginKind][]plugin.PluginIdentifier
}
...
func NewRegistry(dir string, logger logrus.FieldLogger, logLevel logrus.Level) Registry {
	return &registry{
		...
		pluginsByID:    make(map[kindAndName]plugin.PluginIdentifier),
		pluginsByKind:  make(map[plugin.PluginKind][]plugin.PluginIdentifier),
	}
}
...
// PluginKind.
func (r *registry) List(kind plugin.PluginKind) []plugin.PluginIdentifier {
...
func (r *registry) Get(kind plugin.PluginKind, name string) (plugin.PluginIdentifier, error) {
	p, found := r.pluginsByID[kindAndName{kind: kind, name: name}]
	if !found {
		return plugin.PluginIdentifier{}, newPluginNotFoundError(kind, name)
	}
	return p, nil
}
...
func (r *registry) listPlugins(command string) ([]plugin.PluginIdentifier, error) {
	...
	p, err := process.dispense(kindAndName{kind: plugin.PluginKindPluginLister})
	if err != nil {
		return nil, err
	}

	lister, ok := p.(plugin.PluginLister)
	if !ok {
		return nil, errors.Errorf("%T is not a PluginLister", p)
	}
...
func (r *registry) register(id plugin.PluginIdentifier) error {
...
func newPluginNotFoundError(kind plugin.PluginKind, name string) *pluginNotFoundError {
...
type duplicatePluginRegistrationError struct {
	existing  plugin.PluginIdentifier
	duplicate plugin.PluginIdentifier
}

func newDuplicatePluginRegistrationError(existing, duplicate plugin.PluginIdentifier) *duplicatePluginRegistrationError {

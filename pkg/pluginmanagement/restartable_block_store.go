...
func (r *restartableBlockStore) reinitialize(dispensed interface{}) error {
	blockStore, ok := dispensed.(volumeinterface.BlockStore)
	if !ok {
...
// plugin process.
func (r *restartableBlockStore) getBlockStore() (volumeinterface.BlockStore, error) {
	plugin, err := r.sharedPluginProcess.getByKindAndName(r.key)
	if err != nil {
		return nil, err
	}

	blockStore, ok := plugin.(volumeinterface.BlockStore)
	if !ok {
...
func (r *restartableBlockStore) getDelegate() (volumeinterface.BlockStore, error) {
...
func (r *restartableBlockStore) init(blockStore volumeinterface.BlockStore, config map[string]string) error {

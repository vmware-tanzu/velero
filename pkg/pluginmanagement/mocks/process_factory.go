...
func (_m *ProcessFactory) newProcess(command string, logger logrus.FieldLogger, logLevel logrus.Level) (pluginmanagement.Process, error) {
	ret := _m.Called(command, logger, logLevel)

	var r0 pluginmanagement.Process
	if rf, ok := ret.Get(0).(func(string, logrus.FieldLogger, logrus.Level) pluginmanagement.Process); ok {
		r0 = rf(command, logger, logLevel)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(pluginmanagement.Process)
		}
	}

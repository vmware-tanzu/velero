...
type Manager interface {
	...
	GetObjectStore(name string) (objectinterface.ObjectStore, error)

	// GetBlockStore returns the BlockStore plugin for name.
	GetBlockStore(name string) (volumeinterface.BlockStore, error)

	// GetBackupItemActions returns all backup item action plugins.
	GetBackupItemActions() ([]actioninterface.BackupItemAction, error)

	// GetBackupItemAction returns the backup item action plugin for name.
	GetBackupItemAction(name string) (actioninterface.BackupItemAction, error)

	// GetRestoreItemActions returns all restore item action plugins.
	GetRestoreItemActions() ([]actioninterface.RestoreItemAction, error)

	// GetRestoreItemAction returns the restore item action plugin for name.
	GetRestoreItemAction(name string) (actioninterface.RestoreItemAction, error)
...
func (m *manager) getRestartableProcess(kind plugin.PluginKind, name string) (RestartableProcess, error) {
	m.lock.Lock()
	defer m.lock.Unlock()

	logger := m.logger.WithFields(logrus.Fields{
		"kind": plugin.PluginKindObjectStore.String(),
		"name": name,
	})
...
func (m *manager) GetObjectStore(name string) (objectinterface.ObjectStore, error) {
	restartableProcess, err := m.getRestartableProcess(plugin.PluginKindObjectStore, name)
	if err != nil {
		return nil, err
	}
...
func (m *manager) GetBlockStore(name string) (volumeinterface.BlockStore, error) {
	restartableProcess, err := m.getRestartableProcess(plugin.PluginKindBlockStore, name)
	if err != nil {
		return nil, err
	}
...
func (m *manager) GetBackupItemActions() ([]actioninterface.BackupItemAction, error) {
	list := m.registry.List(plugin.PluginKindBackupItemAction)

	actions := make([]actioninterface.BackupItemAction, 0, len(list))
...
func (m *manager) GetBackupItemAction(name string) (actioninterface.BackupItemAction, error) {
	restartableProcess, err := m.getRestartableProcess(plugin.PluginKindBackupItemAction, name)
	if err != nil {
		return nil, err
	}
...
func (m *manager) GetRestoreItemActions() ([]actioninterface.RestoreItemAction, error) {
	list := m.registry.List(plugin.PluginKindRestoreItemAction)

	actions := make([]actioninterface.RestoreItemAction, 0, len(list))
...
func (m *manager) GetRestoreItemAction(name string) (actioninterface.RestoreItemAction, error) {
	restartableProcess, err := m.getRestartableProcess(plugin.PluginKindRestoreItemAction, name)
	if err != nil {
		return nil, err
	}

...
func (r *restartableObjectStore) reinitialize(dispensed interface{}) error {
	objectStore, ok := dispensed.(objectinterface.ObjectStore)
	if !ok {
...
// plugin process.
func (r *restartableObjectStore) getObjectStore() (objectinterface.ObjectStore, error) {
	plugin, err := r.sharedPluginProcess.getByKindAndName(r.key)
	if err != nil {
		return nil, err
	}

	objectStore, ok := plugin.(objectinterface.ObjectStore)
	if !ok {
...
func (r *restartableObjectStore) getDelegate() (objectinterface.ObjectStore, error) {
...
func (r *restartableObjectStore) init(objectStore objectinterface.ObjectStore, config map[string]string) error {

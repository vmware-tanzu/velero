...
// plugin process.
func (r *restartableBackupItemAction) getBackupItemAction() (actioninterface.BackupItemAction, error) {
	plugin, err := r.sharedPluginProcess.getByKindAndName(r.key)
	if err != nil {
		return nil, err
	}

	backupItemAction, ok := plugin.(actioninterface.BackupItemAction)
	if !ok {
		return nil, errors.Errorf("%T is not a action.BackupItemAction!", plugin)
	}
...
func (r *restartableBackupItemAction) getDelegate() (actioninterface.BackupItemAction, error) {
...
func (r *restartableBackupItemAction) AppliesTo() (actioninterface.ResourceSelector, error) {
...
func (r *restartableBackupItemAction) Execute(item runtime.Unstructured, backup *api.Backup) (runtime.Unstructured, []actioninterface.ResourceIdentifier, error) {
	delegate, err := r.getDelegate()
	if err != nil {
		return nil, nil, err
	}

...
type DataUploadLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.DataUpload, err error)
...
// List lists all DataUploads in the indexer.
func (s *dataUploadLister) List(selector labels.Selector) (ret []*v1alpha1.DataUpload, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.DataUpload))
	})
	return ret, err
}
...
type DataUploadNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.DataUpload, error)
...
// List lists all DataUploads in the indexer for a given namespace.
func (s dataUploadNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.DataUpload, err error) {
...
// Get retrieves the DataUpload from the indexer for a given namespace and name.
func (s dataUploadNamespaceLister) Get(name string) (*v1alpha1.DataUpload, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("dataupload"), name)
	}
	return obj.(*v1alpha1.DataUpload), nil
}

...
func TestStartWatch(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "complte but terminated event not received in time",
			thisPod:       "fak-pod-1",
			thisContainer: "fake-container-1",
			insertPod:     builder.ForPod("velero", "fake-pod-1").Phase(v1.PodSucceeded).Result(),
			insertEventsBefore: []insertEvent{
		...
		{
			name:          "complte but terminated event not received immediately",
			thisPod:       "fak-pod-1",
			thisContainer: "fake-container-1",
			insertPod:     builder.ForPod("velero", "fake-pod-1").Phase(v1.PodSucceeded).Result(),
			insertEventsBefore: []insertEvent{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, test.expectCancel, sw.cancelled)

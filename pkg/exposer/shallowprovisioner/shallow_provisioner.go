...
type ShallowCopyProvisioner interface {
	...
	Evaluate(storageClass *storagev1api.StorageClass) (bool, error)
...
func ShallowCopyTransform(ctx context.Context,  storageClient storagev1.StorageV1Interface, pvc *corev1api.PersistentVolumeClaim) (*corev1api.PersistentVolumeClaim, error)  {
	...
	for _, provisioner := range provisioners {
		isShallowCopyEnabled, err := provisioner.Evaluate(storageClass)
		if err != nil {
			return pvc, err
		}
...
*/
func (p cephFSProvisioner) Evaluate(storageClass *storagev1api.StorageClass) (bool, error) {
	// retrieve the StorageClass and compare the provisioner and parameters
...
func (p cephFSProvisioner) Transform(pvc *corev1api.PersistentVolumeClaim) (*corev1api.PersistentVolumeClaim) {
	...
	transform_pvc.Spec.AccessModes = []corev1api.PersistentVolumeAccessMode{corev1api.ReadWriteMany}
...
*/
func (p scaleProvisioner) Evaluate(storageClass *storagev1api.StorageClass) (bool, error) {
	return true, nil
}

...
func (e *genericRestoreExposer) createRestorePod(ctx context.Context, ownerObject corev1api.ObjectReference, targetPVC *corev1api.PersistentVolumeClaim,
	operationTimeout time.Duration, label map[string]string, selectedNode string, resources corev1api.ResourceRequirements, nodeType string) (*corev1api.Pod, error) {
	restorePodName := ownerObject.Name
	restorePVCName := ownerObject.Name

...
func (p *pvcBackupItemAction) determineVGSClass(
	...
	vgsClassList := &volumegroupsnapshotv1alpha1.VolumeGroupSnapshotClassList{}
...
func (p *pvcBackupItemAction) createVolumeGroupSnapshot(
...
) (*volumegroupsnapshotv1alpha1.VolumeGroupSnapshot, error) {
	...
	p.log.Infof("Created VolumeGroupSnapshot %s/%s for PVC group label %s=%s",
		vgs.Namespace, vgs.Name, vgsLabelKey, vgsLabelValue)

	return vgs, nil
}

func (p *pvcBackupItemAction) waitForVGSAssociatedVS(ctx context.Context, vgs *volumegroupsnapshotv1alpha1.VolumeGroupSnapshot, timeout time.Duration) error {
...
func hasOwnerReference(obj metav1.Object, vgs *volumegroupsnapshotv1alpha1.VolumeGroupSnapshot) bool {
...
func (p *pvcBackupItemAction) updateVGSCreatedVS(vgs *volumegroupsnapshotv1alpha1.VolumeGroupSnapshot, backup *velerov1api.Backup) error {
	ctx := context.TODO()
	ns := vgs.Namespace
...
func (p *pvcBackupItemAction) patchVGSCDeletionPolicy(vgs *volumegroupsnapshotv1alpha1.VolumeGroupSnapshot) error {
	ctx := context.TODO()
...
func (p *pvcBackupItemAction) deleteVGSAndVGSC(vgs *volumegroupsnapshotv1alpha1.VolumeGroupSnapshot) error {
	ctx := context.TODO()
...
func (p *pvcBackupItemAction) getVSForPVC(
	...
	for _, vs := range vsList.Items {
		...
		if vs.Status == nil || vs.Status.VolumeGroupSnapshotName == nil || *vs.Status.VolumeGroupSnapshotName != vgsName {
			continue
		}
		if vs.Labels == nil {
			continue
		}
		if vs.Labels[velerov1api.BackupNameLabel] != backup.Name {
			continue
		}
		if vs.Labels[velerov1api.BackupUIDLabel] != string(backup.UID) {
			continue
		}
		return &vs, nil
	}

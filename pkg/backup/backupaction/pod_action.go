...
// podAction implements BackupItemAction.
type podAction struct {
	log logrus.FieldLogger
}

// NewPodAction creates a new BackupItemAction for pods.
func NewPodAction(logger logrus.FieldLogger) actioninterface.BackupItemAction {
	return &podAction{log: logger}
}

// AppliesTo returns a ResourceSelector that applies only to pods.
func (a *podAction) AppliesTo() (actioninterface.ResourceSelector, error) {
...
func (a *podAction) Execute(item runtime.Unstructured, backup *v1.Backup) (runtime.Unstructured, []actioninterface.ResourceIdentifier, error) {
	...
	for i := range volumes {
		...
		additionalItems = append(additionalItems, actioninterface.ResourceIdentifier{

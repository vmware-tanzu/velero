...
func TestNewBackupCache(t *testing.T) {
	...
	delegate.On("GetAllBackups", "bucket1", "path1").Return(bucket1, nil).Once()

	// should be updated via refresh
	updatedBucket1 := []*v1.Backup{
		test.NewTestBackup().WithName("backup2").Backup,
	}
	delegate.On("GetAllBackups", "bucket1", "path1").Return(updatedBucket1, nil)
	...
	delegate.On("GetAllBackups", "bucket2", "path2").Return(bucket2, nil).Once()

	// should be updated via refresh
	updatedBucket2 := []*v1.Backup{
		test.NewTestBackup().WithName("backup7").Backup,
	}
	delegate.On("GetAllBackups", "bucket2", "path2").Return(updatedBucket2, nil)

	backups, err := c.GetAllBackups("bucket1", "path1")
	assert.Equal(t, bucket1, backups)
	assert.NoError(t, err)

	backups, err = c.GetAllBackups("bucket2", "path2")
...
func TestBackupCacheGetAllBackupsUsesCacheIfPresent(t *testing.T) {
	...
	delegate.On("GetAllBackups", "bucket2", "path2").Return(bucket2, nil)

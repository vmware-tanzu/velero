...
func TestNewLogsCommand(t *testing.T) {
	...
	t.Run("Restore not complete test", func(t *testing.T) {
		...
		err := kbClient.Create(context.Background(), restore, &kbclient.CreateOptions{})
	...
	t.Run("Restore with BSL cacert test", func(t *testing.T) {
		...
		cacertValue, err := cacert.GetCACertFromRestore(context.Background(), kbClient, f.Namespace(), restore)
		require.NoError(t, err)

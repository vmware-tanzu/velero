...
func TestNewLogsCommand(t *testing.T) {
	...
	t.Run("Backup with BSL cacert test", func(t *testing.T) {
		...
		bsl := builder.ForBackupStorageLocation(cmdtest.VeleroNameSpace, bslName).
			...
			Result()
		err := kbClient.Create(context.Background(), bsl, &kbclient.CreateOptions{})
		require.NoError(t, err)
		...
		err = kbClient.Get(context.Background(), kbclient.ObjectKey{Namespace: cmdtest.VeleroNameSpace, Name: backupName}, fetchedBackup)
		require.NoError(t, err)

		// Test the cacert fetching logic directly
		cacertValue, err := cacert.GetCACertFromBackup(context.Background(), kbClient, cmdtest.VeleroNameSpace, fetchedBackup)
		require.NoError(t, err)
		...
		go func() {
			...
			if err := kbClient.List(context.Background(), downloadRequestList, &kbclient.ListOptions{
				...
				for _, dr := range downloadRequestList.Items {
					if dr.Spec.Target.Kind == velerov1api.DownloadTargetKindBackupLog &&
						...
						kbClient.Update(context.Background(), &dr)
					}
				}
			}
		}()
...
func TestBSLCACertBehavior(t *testing.T) {
	t.Run("Backup with BSL without cacert test", func(t *testing.T) {
		...
		cacertValue, err := cacert.GetCACertFromBackup(context.Background(), kbClient, cmdtest.VeleroNameSpace, fetchedBackup)
		require.NoError(t, err)
		assert.Equal(t, "", cacertValue, "BSL cacert should be empty when not configured")
	...
	t.Run("Backup with nonexistent BSL test", func(t *testing.T) {
		...
		cacertValue, err := cacert.GetCACertFromBackup(context.Background(), kbClient, cmdtest.VeleroNameSpace, fetchedBackup)
		require.NoError(t, err)
		assert.Equal(t, "", cacertValue, "BSL cacert should be empty when BSL doesn't exist")

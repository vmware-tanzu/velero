		...
		t.Run(test.name, func(t *testing.T) {
			...
			for _, location := range test.locations {
				...
				for _, bucket := range test.cloudBuckets[location.Spec.ObjectStorage.Bucket] {
					...
					backupStore.On("GetBackupPodVolumes", bucket.backup.Name).Return(bucket.podVolumeBackups, nil)
				}
...
func TestDeleteOrphanedPodVolumeBackups(t *testing.T) {
	tests := []struct {
		name                  string
		cloudPodVolumeBackups sets.String
		k8sPodVolumePackups   []*velerov1api.PodVolumeBackup
		namespace             string
		expectedDeletes       sets.String
	}{
		{
			name:                  "no overlapping pod volume backups",
			namespace:             "ns-1",
			cloudPodVolumeBackups: sets.NewString("pvb-1", "pvb-2", "pvb-3"),
			k8sPodVolumePackups: []*velerov1api.PodVolumeBackup{
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-A").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-B").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-C").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
			},
			expectedDeletes: sets.NewString("pvb-A", "pvb-B", "pvb-C"),
		},
		{
			name:                  "some overlapping pod volume backups",
			namespace:             "ns-1",
			cloudPodVolumeBackups: sets.NewString("pvb-1", "pvb-2", "pvb-3"),
			k8sPodVolumePackups: []*velerov1api.PodVolumeBackup{
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-1").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-2").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-C").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
			},
			expectedDeletes: sets.NewString("pvb-C"),
		},
		{
			name:                  "all overlapping pod volume backups",
			namespace:             "ns-1",
			cloudPodVolumeBackups: sets.NewString("pvb-1", "pvb-2", "pvb-3"),
			k8sPodVolumePackups: []*velerov1api.PodVolumeBackup{
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-1").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-2").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-3").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
			},
			expectedDeletes: sets.NewString(),
		},
		{
			name:                  "no overlapping pod volume backups but including ones that are not complete",
			namespace:             "ns-1",
			cloudPodVolumeBackups: sets.NewString("pvb-1", "pvb-2", "pvb-3"),
			k8sPodVolumePackups: []*velerov1api.PodVolumeBackup{
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-A").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("Failed").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseFailed).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("InProgress").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseInProgress).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("New").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseNew).PodVolumeBackup(),
			},
			expectedDeletes: sets.NewString("pvb-A"),
		},
		{
			name:                  "all overlapping pod volume backups and including ones that are not complete",
			namespace:             "ns-1",
			cloudPodVolumeBackups: sets.NewString("pvb-1", "pvb-2", "pvb-3"),
			k8sPodVolumePackups: []*velerov1api.PodVolumeBackup{
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-1").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseFailed).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-2").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseInProgress).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-3").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseNew).PodVolumeBackup(),
			},
			expectedDeletes: sets.NewString(),
		},
		{
			name:                  "no pod volume backups in other locations are deleted",
			namespace:             "ns-1",
			cloudPodVolumeBackups: sets.NewString("pvb-1", "pvb-2", "pvb-3"),
			k8sPodVolumePackups: []*velerov1api.PodVolumeBackup{
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-1").Labels(velerov1api.StorageLocationLabel, "default").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
				...
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-4").Labels(velerov1api.StorageLocationLabel, "alternate").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-5").Labels(velerov1api.StorageLocationLabel, "alternate").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
				defaultPodVolumeBackup().Namespace("ns-1").Name("pvb-6").Labels(velerov1api.StorageLocationLabel, "alternate").Phase(velerov1api.PodVolumeBackupPhaseCompleted).PodVolumeBackup(),
			},
			expectedDeletes: sets.NewString("pvb-C"),
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, podVolumeBackup := range test.k8sPodVolumePackups {
				// add test pod volume backup to informer
				require.NoError(t, sharedInformers.Velero().V1().PodVolumeBackups().Informer().GetStore().Add(podVolumeBackup), "Error adding pod volume backup to informer")

				// add test pod volume backup to client
				_, err := client.VeleroV1().PodVolumeBackups(test.namespace).Create(podVolumeBackup)
				require.NoError(t, err, "Error adding backup to clientset")

				// if we expect this pod volume backup to be deleted, set up the expected DeleteAction
				if test.expectedDeletes.Has(podVolumeBackup.Name) {
					actionDelete := core.NewDeleteAction(
						velerov1api.SchemeGroupVersion.WithResource("podvolumebackups"),
						test.namespace,
						podVolumeBackup.Name,
					)
					expectedDeleteActions = append(expectedDeleteActions, actionDelete)
				}
			}

			c.deleteOrphanedPodVolumeBackups("default", test.cloudPodVolumeBackups, velerotest.NewLogger())

			numPodVolumeBackups, err := numPodVolumeBackups(t, client, c.namespace)
			assert.NoError(t, err)

			expected := len(test.k8sPodVolumePackups) - len(test.expectedDeletes)
			assert.Equal(t, expected, numPodVolumeBackups)

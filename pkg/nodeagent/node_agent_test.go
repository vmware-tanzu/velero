...
func TestIsRunningInNode(t *testing.T) {
	...
	nodeAgentPodRunning3 := builder.ForPod("fake-ns", "fake-pod-3").
		...
		NodeName("fake-node").
		Result()
	nodeAgentPodTerminating := builder.ForPod("fake-ns", "fake-pod-terminating").
		Labels(map[string]string{"role": "node-agent"}).
	...
	tests := []struct {
		...
		},
		{
			...
			nodeName: "fake-node",
			kubeClientObj: []runtime.Object{
				nodeAgentPodTerminating,
				nodeAgentPodNotRunningOnNode,
			},
			expectErr: "daemonset pod not found in running state in node fake-node, err:",
		},
		{

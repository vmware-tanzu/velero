...
// Foo restarts the plugin's process if needed, then delegates the call.
func (r *RestartableBackupItemAction) Foo() error {
	delegate, err := r.getDelegate()
	if err != nil {
		return err
	}
...
// Execute delegates to the v1 Execute call.
func (r *AdaptedV1RestartableBackupItemAction) Execute(item runtime.Unstructured, backup *api.Backup) (runtime.Unstructured, []velero.ResourceIdentifier, error) {
	return r.V1Restartable.Execute(item, backup)
}

// Foo just returns without error since v1 plugins don't implement it.
func (r *AdaptedV1RestartableBackupItemAction) Foo() error {
	return nil
}

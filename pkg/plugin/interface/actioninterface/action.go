...
type BackupItemAction interface {
	...
	// An BackupItemAction's Execute function will only be invoked on items that match the returned
...
type RestoreItemAction interface {
	// AppliesTo returns information about which resources this action should be invoked for.
	// An RestoreItemAction's Execute function will only be invoked on items that match the returned
	// selector. A zero-valued ResourceSelector matches all resources.
	AppliesTo() (ResourceSelector, error)

	// Execute allows the RestoreItemAction to perform arbitrary logic with the item being restored,

...
var _ = Describe("[Azure] [Kind] [Restic] Velero tests on Kind cluster using Azure provider for object storage", func() {
	var (
	...
	BeforeEach(func() {
		...
		io, err := GetProviderVeleroInstallOptions("azure", cloudCredentialsFile, bslBucket, bslPrefix, bslConfig, vslConfig, getProviderPlugins("azure"))
		Expect(err).NotTo(HaveOccurred(), "Failed to get Velero InstallOptions for provider azure")
		io.UseRestic = true
	...
	Describe("Run Kibishii backup and restore test using Azure provider and Restic for volume backup", func() {
		Context("should install kibishii as test workload", func() {
			It("should successfully backup and restore kibishii workload", func() {
				// Even though we are using Velero's Azure plugin for object storage, the kubernetes cluster is running on

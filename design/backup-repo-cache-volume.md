...
Payload data which is highly related to the backup data could be evaluated based on the backup data, e.g., the total size is related to the size of the backup.  
...
That is to say, backup payload data is the key factor to estimate the cache data size in most cases, but repository metadata might also need to be considered.  
...
- Data Upload for Backup: this is the process to upload/write the backup data into the backup repository, e.g., DataUpload or PodVolumeBackup. The pieces of data is almost directly written to the repository, sometimes with small group and shot time stay in the local place. That is to say, there should not be large scale data cached for this scenario, so we don't prepare dedicated cache for this scenario.
...
The above analyses are based on the common behavior of backup repositories and existing, as a conclusion of them, we will create dedicated cache volumes for restore scenarios. For other scenarios, we can add it regarded to the future changes/requirements. The mechanism to expose and connect the cache volumes should work for all scenarios.  
...
The cache volume size is calculated from below factors:  
...
```
Finally, the `cacheVolumeSize` will be rounded up to GiB considering the UX friendlyness, storage friendlyness and management friendlyness.  
...
Cache configurations reuse the existing BackupRepository configMap and ```repositoryConfig``` in the BackupRepository CRD. So the storing and retrieving mechanism are kept as is, and the configurations are also per-repository. For more details, see [Backup Repository Configuration][5].  

Specifically, below configurations are involved:
- **cacheLimitMB**: the existing `cacheLimitMB`, which specifies the size limit(in MB) for the local data cache 
- **residentThresholdMB**: the minimum backup size(in MB) that a cache volume is created
...
```
**repository-type-1**: This is a valid configuration. Restores with backup data size larger than 1G will be assigned a cache volume using storage class sc-1, the cache limit is 5G.  
**repository-type-2**: This is a valid configuration. Restores are always assigned a cache volume using storage class sc-1, the cache limit is 5G.  
...
**repository-type-4**: This is a valid configuration. Restores of this backup repository are never assigned a cache volume.  
...
The cache volume configurations will be visited by node-agent server, so they also need to specify the `--repo-maintenance-job-configmap` to the `velero node-agent` parameters.  

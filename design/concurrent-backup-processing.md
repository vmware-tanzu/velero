...
- (Phase 2) Item-level overlap detection
...
In addition to namespace-level conflict detection, which prevents backups which have included namespaces in common from running concurrently, we will add ItemBlock-level conflict detection, which will prevent two different ItemBlock workers from processing ItemBlocks which have any included items in common at the same time. When namespace-level conflict detection is active, this will primarily affect cluster-scoped resources. If two ItemBlocks contain the same cluster-scoped resource, they will not be processed concurrently.
...
The startup (and shutdown) of the ItemBlock worker pool will be moved from reconciler startup to the backup reconcile, which will give each running backup its own dedicated worker pool.
...
#### ItemBlock model changes

The ItemBlock struct will get a new int field, `requeueCount` which tracks the number of times the ItemBlock has been requeued due to conflict with already-running ItemBlocks. This will facilitate implementing a backoff delay, where each requeue request will wait a longer period of time before requeuing, allowing longer-running conflicts to be resolved without needless too-frequent rechecking.

#### ItemBlockWorkerPool changes

A new synchronized map will be added which will track all items from ItemBlocks currently being backed up. This map will have a combined check/insert function, addItemsIfNoConflict() which takes an ItemBlock as input. If any conflicts are found, it will return false, else it will add the items to the map and return true. Finally, a removeItems() function will take an ItemBlock as input and remove the contained items from the map.

Currently, the ItemBlockWorkerPool takes ItemBlocks directly from the input channel which contains new ItemBlocks from processing backups. This will be replaced with a getItemBlockToProcess() function which will return a requeued ItemBlock if available, or a new ItemBlock if no requeued block is available. In addition, the function requeueItemBlock() will be added which will add the ItemBlock to the requeued channel with some backoff delay for subsequent requeues.

Once the worker gets an ItemBlock, before calling kubernetesBackupper.backupItemBlock, it first makes a synchronized call to see whether any of the items in the ItemBlock are currently being backed up by other workers. If any overlap is found, then the ItemBlock is requeued via `requeueItemBlock() and a new ItemBlock is obtained from the list. The worker continues this process until an ItemBlock without conflict is found. At this point, the items have been added to the "in progress" map and the worker will call kubernetesBackupper.backupItemBlock() When this function returns, the worker will call removeItems() before returning the response via the return channel.

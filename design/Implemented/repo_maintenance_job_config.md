...
This design reuses the data structure introduced by design [Velero Generic Data Path affinity configuration](VGDP-affinity.md) to make the repository maintenance job can choose which node running on.
...
The `LoadAffinity` structure is reused from design [Velero Generic Data Path affinity configuration](VGDP-affinity.md).

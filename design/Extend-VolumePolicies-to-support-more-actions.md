...
type VolumeHelper interface {
	ShouldPerformFSBackupForPodVolume(pod *corev1api.Pod) (bool, error)
...
```go
type VolumePolicyHelper struct {
...
- We will create an instance of the struct the `VolumePolicyHelper` in `item_backupper.go`
```go
	vph := &volumehelper.VolumePolicyHelper{
...
- Regarding `fs-backup` action to decide whether to use legacy annotation based approach or volume policy based approach:
  - We will use the `vph.ProcessVolumePolicyFSBackup` function from the `volumehelper` package
...
func (v *VolumePolicyHelper) ProcessVolumePolicyFSBackup(pod *corev1api.Pod, volumePolicy *resourcepolicies.Policies, defaultVolumesToFsBackup bool, backupExcludePVC bool) ([]string, []string, error) {

	var includedVolumes = make([]string, 0)
	var optedOutVolumes = make([]string, 0)

	FSBackupOrSnapshot, err := v.ShouldPerformFSBackupForPodVolume(pod)
	if err != nil {
...
func (v *VolumePolicyHelper) ShouldPerformFSBackupForPodVolume(pod *corev1api.Pod) (bool, error) {
...
```go
includedVolumes, optedOutVolumes, err := vph.ProcessVolumePolicyFSBackup(pod, vph.VolumePolicy, vph.DefaultVolumesToFsBackup, vph.BackupExcludePVC)
			if err != nil {
...
- Making sure that `snapshot` action is skipped for PVs that do not fit the volume policy criteria, for this we will use the `vph.ShouldPerformSnapshot` from the `VolumePolicyHelper(vph)` receiver.
```go
func (v *VolumePolicyHelper) ShouldPerformSnapshot(obj runtime.Unstructured) (bool, error) {
	// Here check if the object(pv/pvc) fits a volume policy criteria and see if the associated action is snapshot
...
```go
snapshotVolumePolicy, err := vhp.ShouldPerformSnapshot(obj)
...
if groupResource == kuberesource.PersistentVolumeClaims && actionName == csiBIAPluginName {
			snapshotVolumePolicy, err := vph.ShouldPerformSnapshot(obj)
			if err != nil {
